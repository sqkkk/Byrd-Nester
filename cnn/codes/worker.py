...
class TorchWorker(object):
    ...
    def get_current_gradient(self) -> torch.Tensor:
        return self._get_current_grad()
    ...
    def _get_current_grad(self) -> torch.Tensor:
        layer_gradients = []
        for group in self.optimizer.param_groups:
            for p in group["params"]:
                param_state = self.state[p]
...
class MomentumWorker(TorchWorker):
    def __init__(self, momentum, nesterov, *args, **kwargs):
        ...
        for group in self.optimizer.param_groups:
            for p in group["params"]:
                ...
                if "momentum_buffer" not in param_state:
                    ...
                    param_state["momentum_buffer"].mul_(self.momentum).add_(p.grad, alpha = 1 - self.momentum)
    ...
    def _get_saved_grad(self) -> torch.Tensor:
        ...
        for group in self.optimizer.param_groups:
            for p in group["params"]:
                param_state = self.state[p]
                if self.nesterov:#修改
                    p.grad.data.mul_(1 - self.momentum).add_(self.momentum, param_state["momentum_buffer"])
                else:
                    p.grad = torch.clone(param_state["momentum_buffer"]).detach()
...
class ByzantineWorker(TorchWorker):
    ...
    def get_current_gradient(self) -> torch.Tensor:
        # Use self.simulator to get all other workers
        return super().get_current_gradient()
