...
def create_noniid_data(x, y, num_clients, noniid_degree=0.1):
    ...
    for i in range(num_clients):
        ...
        for j in range(num_classes):
            # 每个类分给每个客户端的数据量，根据non-IID程度进行调整
...
class Optimizer:
    ...
    def update_weights_nesterov_lookahead(self, alpha, model, data_splits, start, end, last_grads, last_grads_hat, agg_type, regular_nodes, attack_type, grads_w, grads_b):
        ...
        self.v_w = alpha * grad_w_agg + (1-alpha) * (self.momentum * self.v_w + (1) * grad_w_agg_hat)
        self.v_b = alpha * grad_b_agg + (1-alpha) * (self.momentum * self.v_b + (1) * grad_b_agg_hat)
...
# 分布式训练函数
def distributed_train_model(alpha, model, optimizer, X_train, y_train, X_test, y_test, epochs, num_nodes, batch_size, agg_type, attack_type, method):
    np.random.seed(2024)
    train_loss_history = []
    test_loss_history = []
    attacked_nodes = [8,9]
    ...
    for epoch in range(epochs):
        ...
        for i in range(int(num_batches)):
            ...
            if method == 'sgd':
                ...
                # attack_type = 10
                model.weights, model.bias, last_grads, last_grads_hat, grads_w, grads_b = optimizer.update_weights_nesterov_lookahead(alpha, model, data_splits, start, end, last_grads, last_grads_hat, agg_type, regular_nodes, attack_type, grads_w, grads_b)
...
for label_att, attack_type in attack_types.items():
    ...
    for label_agg, agg_type in agg_types.items():
        ...
        # with open('logr_outputsgd-new-3.txt', 'a') as file:
        ...
        # with open('logr_outputsgdm-new-3.txt', 'a') as file:
        #     file.write("(" + str(i)+','+ str(train_loss_momentum[-1]) + ')' +'\n')
            ...
            train_loss_nesterov_lookahead, test_loss_nesterov_lookahead = distributed_train_model(alpha, model_nesterov_lookahead,
            ...
            plt.plot(train_loss_nesterov_lookahead, label=r'$\alpha$'+'='+str(alpha))
        # with open('logr_outputsgdn-new-3.txt', 'a') as file:
        #     file.write("(" + str(i)+','+ str(train_loss_nesterov_lookahead[-1]) + ')' +'\n')

# 绘制损失曲线
# plt.plot(train_loss_sgd, label='DSGD')
# plt.plot(train_loss_momentum, label='DSGDm')
# plt.plot(train_loss_nesterov_lookahead, label='Algorithm 1')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.title('Loss Curves for different '+r'$\alpha$')
plt.yscale('log')
filename = 'Loss Curves for alpha'+'.png'
